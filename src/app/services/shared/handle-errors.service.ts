...
export class HandleErrorsService {
  ...
  handleErrors(err: HttpErrorResponse, childLabel?: string): string {
    ...
    switch (err.status) {
      case 0:
        this.errorMessage = (err.error) ? err.error.message : this.translate.get(generalKeyLabel + '.cantConnect')
      ...
      case 400:
        ...
        break;
      case 403:
        this.translate.get(generalKeyLabel.concat(`.${(childLabel) ? childLabel : '.authorization'}`))
          .subscribe((res: string) => this.errorMessage = res);
        break;
      case 404:
